...
function run_sim_dynamic_suggester(
    problem::Symbol,
    type_problem::Symbol;
    num_trials::Int=1,
    max_steps::Int=50,
    ...
    init_pos=nothing,
    seed=42,
    suggesters::Dict{Int, <:AbstractSuggester}=Dict(0 => NoSuggester(0.0)),
    ν::Float64=1.0
)
    ...
    # for ijk = 1:num_sims
        ...
        suggs_thread = deepcopy(suggesters)
        
        sorted_sugg_keys = sort(collect(keys(suggesters)))
        @assert sorted_sugg_keys[1] == 0 "Suggesters must have a key of 0 for a start time."
        ...
        try
            ...
            while trial_cnt < num_trials
                ...
                # Get the suggester for this time step. The largest suggester key that is 
                # not greater than than t
                sugg_key = sorted_sugg_keys[findlast(x -> x <= trial_cnt, sorted_sugg_keys)]
                sugg_thread = suggs_thread[sugg_key]
                # @info "Trial: $trial_cnt, Suggestion Key: $sugg_key"
